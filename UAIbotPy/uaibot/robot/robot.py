...
class Robot:
  ...
  """
    ...
    def compute_jrdot(self, htm_tg, q=None, qdot=None, htm=None):
        """
        Compute the task Jacobian and its time derivative analytically in C++.

        Parameters
        ----------
        htm_tg : array-like
            Target end-effector homogeneous transformation.
        ...
        jrdot : numpy.ndarray
            Time derivative of the task Jacobian.
        """
        ...
        if os.environ['CPP_SO_FOUND'] == '0':
            raise Exception("The C++ extension is required to compute Jrdot.")

        task_dot_res = self.cpp_robot.task_function_dot(
            Utils.cvt(q),
            Utils.cvt(qdot),
            Utils.cvt(htm),
            Utils.cvt(htm_tg)
        ...
        return (
            np.asarray(task_dot_res.task, dtype=float).reshape((6, 1)),
            np.asarray(task_dot_res.jac_task, dtype=float),
            np.asarray(task_dot_res.jac_task_dot, dtype=float)
        )
